...
func Benchmark64JSON(b *testing.B) {
	...
	mv := Movement64{
		Opcode:      64,
		CharacterID: [64]byte(characterID),
		X:           13.34,
		Y:           20.36,
		Z:           45.13,
	}
...
func Benchmark64YAMLStrict(b *testing.B) {
	rand.Seed(64)
	var randomData [64]byte
	rand.Read(randomData[:])
	characterID := randomData
...
func Benchmark64BinaryNoReflection(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		copy(newMv.CharacterID[:], data[4:68])
		newMv.X = math.Float64frombits(binary.LittleEndian.Uint64(data[68:]))
		newMv.Y = math.Float64frombits(binary.LittleEndian.Uint64(data[76:]))
		newMv.Z = math.Float64frombits(binary.LittleEndian.Uint64(data[84:]))
	}
}
...
func Benchmark64BinaryBigEndianNoReflection(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		copy(newMv.CharacterID[:], data[4:68])
		newMv.X = math.Float64frombits(binary.BigEndian.Uint64(data[68:]))
		newMv.Y = math.Float64frombits(binary.BigEndian.Uint64(data[76:]))
		newMv.Z = math.Float64frombits(binary.BigEndian.Uint64(data[84:]))
	}
}
...
func Benchmark64Unsafe(b *testing.B) {
	...
	data := (*[Movement64Size]byte)(unsafe.Pointer(&mv))[:]

	for i := 0; i < b.N; i++ {
		_ = *(*Movement64)(unsafe.Pointer(&data[0]))
	}
}