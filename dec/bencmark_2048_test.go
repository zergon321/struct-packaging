...
func Benchmark2048JSON(b *testing.B) {
	...
	mv := Movement2048{
		Opcode:      2048,
		CharacterID: [2048]byte(characterID),
		X:           13.34,
		Y:           20.36,
		Z:           45.13,
	}
...
func Benchmark2048YAMLStrict(b *testing.B) {
	rand.Seed(2048)
	var randomData [2048]byte
	rand.Read(randomData[:])
	characterID := randomData
...
func Benchmark2048BinaryNoReflection(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		copy(newMv.CharacterID[:], data[4:2052])
		newMv.X = math.Float64frombits(binary.LittleEndian.Uint64(data[2052:]))
		newMv.Y = math.Float64frombits(binary.LittleEndian.Uint64(data[2060:]))
		newMv.Z = math.Float64frombits(binary.LittleEndian.Uint64(data[2068:]))
	}
}
...
func Benchmark2048BinaryBigEndianNoReflection(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		copy(newMv.CharacterID[:], data[4:2052])
		newMv.X = math.Float64frombits(binary.BigEndian.Uint64(data[2052:]))
		newMv.Y = math.Float64frombits(binary.BigEndian.Uint64(data[2060:]))
		newMv.Z = math.Float64frombits(binary.BigEndian.Uint64(data[2068:]))
	}
}
...
func Benchmark2048Unsafe(b *testing.B) {
	...
	data := (*[Movement2048Size]byte)(unsafe.Pointer(&mv))[:]

	for i := 0; i < b.N; i++ {
		_ = *(*Movement2048)(unsafe.Pointer(&data[0]))
	}
}