...
func Benchmark4096JSON(b *testing.B) {
	...
	mv := Movement4096{
		Opcode:      4096,
		CharacterID: [4096]byte(characterID),
		X:           13.34,
		Y:           20.36,
		Z:           45.13,
	}
...
func Benchmark4096YAMLStrict(b *testing.B) {
	rand.Seed(4096)
	var randomData [4096]byte
	rand.Read(randomData[:])
	characterID := randomData
...
func Benchmark4096BinaryNoReflection(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		copy(newMv.CharacterID[:], data[4:4100])
		newMv.X = math.Float64frombits(binary.LittleEndian.Uint64(data[4100:]))
		newMv.Y = math.Float64frombits(binary.LittleEndian.Uint64(data[4108:]))
		newMv.Z = math.Float64frombits(binary.LittleEndian.Uint64(data[4116:]))
	}
}
...
func Benchmark4096BinaryBigEndianNoReflection(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		copy(newMv.CharacterID[:], data[4:4100])
		newMv.X = math.Float64frombits(binary.BigEndian.Uint64(data[4100:]))
		newMv.Y = math.Float64frombits(binary.BigEndian.Uint64(data[4108:]))
		newMv.Z = math.Float64frombits(binary.BigEndian.Uint64(data[4116:]))
	}
}
...
func Benchmark4096Unsafe(b *testing.B) {
	...
	data := (*[Movement4096Size]byte)(unsafe.Pointer(&mv))[:]

	for i := 0; i < b.N; i++ {
		_ = *(*Movement4096)(unsafe.Pointer(&data[0]))
	}
}