...
func Benchmark128BinaryNoReflection(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		copy(newMv.CharacterID[:], data[4:132])
		newMv.X = math.Float64frombits(binary.LittleEndian.Uint64(data[132:]))
		newMv.Y = math.Float64frombits(binary.LittleEndian.Uint64(data[140:]))
		newMv.Z = math.Float64frombits(binary.LittleEndian.Uint64(data[148:]))
	}
}
...
func Benchmark128BinaryBigEndianNoReflection(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		copy(newMv.CharacterID[:], data[4:132])
		newMv.X = math.Float64frombits(binary.BigEndian.Uint64(data[132:]))
		newMv.Y = math.Float64frombits(binary.BigEndian.Uint64(data[140:]))
		newMv.Z = math.Float64frombits(binary.BigEndian.Uint64(data[148:]))
	}
}
